...
export function rentCreditCliffsDigest() {
  ...
  return {
    ...
    table: {
      head: ["총급여", "공제율 (소득세)", "월세 세액공제 (소득세)", "결정세액 (소득세, 공제 전)", "실제 환급되는 몫 (소득세)"],
      rows: rows.map((row) => ({
    ...
    tableNote: `월세 ${won(RENT_SCENARIO.monthlyRent)}·12개월, 부양가족 1인·비과세 식대 월 20만원 기준이며 결정세액은 근로소득세액공제와 표준세액공제만 반영한 값입니다. 이 표는 전부 소득세 기준이라 위쪽 표의 지방소득세 포함 절세액보다 작습니다 — 각 칸에 10%를 더하면 실제로 덜 내는 금액이 됩니다. 다른 세액공제(연금계좌·보험료·의료비)를 함께 받으면 월세 공제에 쓸 수 있는 결정세액이 그만큼 줄어듭니다.`,
  };
}
...
export function rentCreditCapDigest() {
  ...
  return {
    ...
    table: {
      head: ["월세", "연 월세", "공제 대상 월세", "세액공제 (소득세)", "실효 공제율", "돌려받는 월세 개월치"],
      rows: rents.map((row) => ({
