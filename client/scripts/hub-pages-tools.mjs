...
function allHub() {
  ...
  return {
    ...
    sections: [
      ...
      // --- 아래 다섯 절은 홈(/)에서 옮겨 온 것이다 ---------------------------------
      // 홈은 26개 계산기로 가는 인덱스 화면이어야 하는데, 이 안내들이 h2를 하나씩 달고
      // 목록 아래에 붙어 있어 h2가 10개인 긴 글이 되어 있었다. 내용은 버리지 않고 목록을
      // 이미 다 펼쳐 놓은 이 페이지로 옮긴다 — 계산기를 고른 사람이 다음으로 묻는 것이
      // "이 숫자를 어디까지 믿나"이기 때문이다.
      //
      // 홈의 "2026년 계산에 적용된 기준" 절은 옮기지 않았다. 바로 위 절이 같은 수치를
      // 같은 순서로 이미 말하고 있어, 옮기면 이관이 아니라 한 페이지 안의 중복이 된다.
      {
        h2: "계산 근거와 한계",
        body: [
          "국세청 근로소득 간이세액표, 국민건강보험공단 보험료 고시, 국민연금공단 기준소득월액 고시, 고용노동부 고시를 근거로 계산합니다. 다만 회사가 잡아 둔 비과세 항목, 중도 입·퇴사, 성과급 지급 시점에 따라 실제 급여명세서와는 차이가 날 수 있습니다.",
          "결과는 법적 효력이 없는 참고용 추정값이며, 확정 금액은 급여명세서와 원천징수영수증으로 확인하세요.",
        ],
      },
      {
        h2: "회원가입도, 설치도 없이",
        body: [
          "모든 계산기는 브라우저에서 바로 동작합니다. 입력한 급여·세금 정보는 서버로 전송되지 않고 브라우저 안에서만 처리되므로 민감한 급여 정보를 남기지 않고 쓸 수 있습니다. 운영비는 광고로 충당하며 사용자에게 과금하지 않습니다.",
        ],
      },
      {
        h2: "자주 찾는 금액은 미리 계산해 두었습니다",
        body: [
          `연봉 3,000·4,000·5,000만원, 건보료 10만·14만·20만원처럼 검색이 많은 값은 고정 주소를 가진 개별 페이지로 만들어 두었습니다. 주소를 열면 그 값이 채워진 상태로 계산 결과와 해설이 함께 나오므로, 검색 결과에서 바로 답을 확인하거나 링크를 그대로 공유할 수 있습니다. 예를 들어 <a href="/finance/salary/4000">연봉 4,000만원 실수령액</a>이나 <a href="/finance/insurance/140000">건보료 14만원 연봉 계산</a>이 그런 주소입니다.`,
        ],
      },
      {
        h2: "여러 계산기를 순서대로 써야 할 때",
        body: [
          "상황 하나를 판단하려면 계산기 한 개로는 부족한 경우가 많습니다. 이직이라면 제안 연봉의 세후 금액, 두 회사의 실수령 차이, 4대보험 부담 변화를 차례로 봐야 하고, 퇴사라면 퇴직금과 실업급여를 먼저 구한 뒤 퇴사 후 건강보험료를 빼야 실제로 버틸 수 있는 기간이 나옵니다.",
          "이런 흐름은 아래 상황별 가이드에 순서대로 정리해 두었습니다. 이직 준비, 퇴사 준비, 연말정산, 아르바이트 네 가지가 있으며 각 단계마다 어느 계산기를 왜 여는지 설명합니다.",
        ],
      },
      {
        h2: "숫자가 틀리면 알려주세요",
        body: [
          `세율과 요율은 법령 개정과 정부 고시에 따라 연중에도 바뀝니다. 2026년 7월 국민연금 기준소득월액 상한이 ${won(RATES_2026.nationalPension.maxMonthlyIncome)}으로 오른 것처럼, 시행일이 지나면 계산 로직과 안내 문구를 함께 갱신합니다.`,
          "각 계산기 하단에는 적용한 기준과 시행일, 공식 출처 링크를 표시해 두었으니 중요한 신고나 정산 전에는 원문 고시와 교차 확인하세요. 계산 결과에서 오류를 발견하면 문의 메일로 알려주시면 확인 후 수정합니다.",
        ],
      },
    ],
    variants: {
