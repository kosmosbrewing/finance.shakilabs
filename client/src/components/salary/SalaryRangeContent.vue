...
const amountLabel = computed(() => {
  if (props.amount >= 10000) {
    return `${props.amount / 10000}억`;
  }
  return `${props.amount.toLocaleString()}만`;
});
...
const faqItems = computed(() => [
  {
    q: `연봉 ${amountLabel.value}원의 월 실수령액은 얼마인가요?`,
    a: `2026년 기준 연봉 ${amountLabel.value}원(부양가족 ${props.calc.dependents.value}인, 비과세 월 ${formatWon(props.calc.nonTaxableMonthly.value)})의 월 실수령액은 약 ${formatWon(props.calc.monthlyNet.value)}입니다. 4대보험 ${formatWon(props.calc.totalInsurance.value)}과 세금 ${formatWon(props.calc.totalTax.value)}이 공제됩니다.`,
  },
  {
    q: `연봉 ${amountLabel.value}원의 4대보험료는 얼마인가요?`,
  ...
  {
    q: `연봉 ${amountLabel.value}원의 실효세율은 얼마인가요?`,
...
<template>
  <div class="space-y-6">
    <div class="retro-panel retro-panel-content">
      <h2 class="text-h2 mb-3">연봉 {{ amountLabel }}원 실수령액 요약</h2>
      <p class="text-body text-muted-foreground leading-relaxed">
        2026년 최신 세율 기준으로 연봉 {{ amountLabel }}원의 월 실수령액은
    ...
    <div v-if="insights.length" class="retro-panel-muted p-3 sm:p-4 space-y-2">
      <h3 class="text-caption font-semibold text-foreground">연봉 {{ amountLabel }}원 세금 인사이트</h3>
